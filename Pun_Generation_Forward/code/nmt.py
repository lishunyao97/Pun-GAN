...
def run_main(flags, default_hparams, train_fn, inference_fn, sample_decode_fn, target_session=""):
  ...
  if flags.inference_input_file:
    ...
    print('[train]\n')
    wsd_input, senses_input, wsd_src_data, eval_result = sample_decode_fn(hparams, target_session=target_session)

    dict = {'wsd_input': wsd_input, 'senses_input': senses_input}
    print('[wsd_input_dict] = ', dict)
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation_Forward/code/wsd_input.pickle', 'wb') as fw:
        pickle.dump(dict, fw)

    str = ('python ' + PUNGAN_ROOT_PATH + '/WSD/BiLSTM/train.py ' + PUNGAN_ROOT_PATH + '/Pun_Generation_Forward/code/wsd_input.pickle')
    p = os.system(str)
    print(p)
    ...
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation_Forward/data/2forward/forward_index.in', 'w') as f:
        for i in test_data_index:
            f.write(wsd_src_data[i].encode('utf-8',errors='ignore')+'\n')
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation_Forward/data/2forward/forward_index.out', 'w') as f:
        for i in test_data_index:
            f.write(eval_result[i]+'\n')
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation_Forward/data/2forward/wsd_train_reward.in', 'w') as f:
        for i in return_reward:
            f.write("%.5f" % i)
            f.write('\n')
