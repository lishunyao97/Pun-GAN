...
def run_internal_eval(
  ...
  if use_test_set and hparams.test_prefix:
    ...
    test_ppl = _internal_eval(loaded_eval_model, global_step, eval_sess,
                              ...
                              summary_writer, "test")
  with open('backward_ppl.txt', 'a+') as fw:
      fw.write('dev_ppl: ')
...
def run_sample_decode_pungan_prepare(hparams, scope=None, target_session=""):
    ...
    eval_result = run_sample_decode_pungan(infer_model, infer_sess, model_dir, hparams, wsd_src_data)
    ...
    for block in range(len(eval_result)/(2*hparams.sample_size)):
        src_word1, src_word2 = wsd_src_data[2 * block], wsd_src_data[2 * block + 1]
        for sent_id in range(block * hparams.sample_size, (block+1) * hparams.sample_size):
            tgt_sent = src_word1.decode().encode('utf-8') + ' ' + eval_result[sent_id]
...
def train(hparams, scope=None, target_session="", compute_ppl=0):
  ...
  while global_step < end_step: # pre-train: num_train_steps
    ### Run a step ###
...
def _sample_decode_pungan(model, global_step, sess, hparams, iterator, src_data, iterator_src_placeholder, iterator_batch_size_placeholder):
  ...
  sess.run(iterator.initializer, feed_dict=iterator_feed_dict)
  infer_logits1, nmt_outputs1, attention_summary1 = model.decode(sess)

  print('nmt_outputs0.shape', nmt_outputs.shape)
  print('nmt_outputs1.shape', nmt_outputs1.shape)
  print('nmt_outputs', nmt_outputs)
  print('nmt_outputs1', nmt_outputs1)
  if hparams.beam_width > 0:
    # get the top translation.
    nmt_outputs = nmt_outputs[0]
  length = int(hparams.infer_batch_size * hparams.sample_size * 0.5) # length=256
  def process(nmt_outputs):
      return_list = []
      for sent_id in range(hparams.infer_batch_size * hparams.sample_size):
          tgt_eos = hparams.eos
      ...
      return return_list
  return_list = process(nmt_outputs)[:length] + process(nmt_outputs1)[:length]
