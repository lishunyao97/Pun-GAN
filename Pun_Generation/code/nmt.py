...
# from BiLSTM import train as wsd_train
...
def add_arguments(parser):
  ...
  parser.add_argument("--num_train_steps", type=int, default=1200000, help="Num steps to train.")
  ...
  parser.add_argument("--sample_prefix", type=str, default=None,
                          help="Sample prefix, expect files with src/tgt suffixes.")
  parser.add_argument("--reward_prefix", type=str, default=None,
                          help="Reward prefix, expect files with src/tgt suffixes.")
  parser.add_argument("--first_step", type=str, default=None,
                          help="Tell first step or second step?")
  ...
  parser.add_argument("--pretrain", type=str, default=None,
                                        help="pretrain")
  parser.add_argument("--compute_ppl", type=int, default=0, help="internal evaluation")
  # Vocab
  ...
  parser.add_argument("--inference_list", type=str, default=None,
                      help=("A comma-separated list of sentence indices "
                            "(0-based) to decode."))
  parser.add_argument("--infer_batch_size", type=int, default=200,
  ...
  parser.add_argument("--sampling_temperature", type=float,
      ...
      """))
  parser.add_argument("--num_translations_per_input", type=int, default=10,
                      help=("""\
...
def create_hparams(flags):
  ...
  return tf.contrib.training.HParams(
      ...
      sample_prefix=flags.sample_prefix,
      reward_prefix=flags.reward_prefix,
      first_step=flags.first_step,
      vocab_prefix=flags.vocab_prefix,
      embed_prefix=flags.embed_prefix,
      out_dir=flags.out_dir,
      pretrain=flags.pretrain,
      compute_ppl=flags.compute_ppl,
...
def run_main(flags, default_hparams, train_fn, inference_fn, sample_decode_fn, target_session=""):
  """Run main."""
  # Job
  jobid = flags.jobid
  ...
  if flags.inference_input_file:
    print('[flags.inference_input_file]\n')
    # Inference indices
    ...
    train_fn(hparams, target_session=target_session, compute_ppl=flags.compute_ppl)
  elif flags.first_step:
    # first step: sample

    print('[train]\n')
    wsd_src_data_new, eval_result_new = sample_decode_fn(hparams, target_session=target_session)
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation/data/1backward/backward_step1.in', 'w') as f:
        for i in wsd_src_data_new:
            f.write(i+'\n')
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation/data/1backward/backward_step1.out', 'w') as f:
        for i in eval_result_new:
            f.write(i+'\n')

  else:
    # second step: RL train
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation_Forward/code/wsd_output.pickle', 'rb') as f:
        out_dict = pickle.load(f)
    return_reward = out_dict['return_reward']
    test_data_index = out_dict['test_data_index']
    ...
    for i in range(len(words)/2):
        for j in range(32):
            wsd_src_data_new.append(words[i*2+j%2])
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation/data/1backward/backward_step1.out') as f:
        for line in f:
            eval_result_new.append(line.strip())
    print('len(wsd_src_data_new)',len(wsd_src_data_new))
    print('len(eval_result_new)',len(eval_result_new))
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation/data/1backward/backward_step2.in', 'w') as f:
        for i in test_data_index:
            f.write(wsd_src_data_new[i]+'\n')
    with open(PUNGAN_ROOT_PATH + '/Pun_Generation/data/1backward/backward_step2.out', 'w') as f:
        for i in test_data_index:
            f.write(eval_result_new[i]+'\n')
    if len(return_reward) > 0:
...
def main(unused_argv):
  ...
  sample_decode_fn = train.run_sample_decode_pungan_prepare
  run_main(FLAGS, default_hparams, train_fn, inference_fn, sample_decode_fn)
