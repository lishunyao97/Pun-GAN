...
def decode_and_evaluate(name,
  ...
  if decode:
    ...
    with codecs.getwriter("utf-8")(
      ...
      #num_translations_per_input = max(min(10, beam_width), 1)
      print ("num_translations_per_input", num_translations_per_input) 
      while True:
        try:
          infer_logits, nmt_outputs, _ = model.decode(sess)
          #print (infer_logits)
          if beam_width == 0:
          ...
          #print ("infer_logits",infer_logits)
          #print ("nmt_outputs",nmt_outputs)
          for sent_id in range(batch_size):
            for beam_id in range(num_translations_per_input):
              #print ("nmt_outputs[beam_id]",nmt_outputs[beam_id])
              #print ("infer_logits[beam_id]",infer_logits[beam_id])
              translation = get_translation(
                  nmt_outputs[beam_id],
                  infer_logits[beam_id],
                  sent_id,
                  tgt_eos=tgt_eos,
...
def get_translation_train(nmt_outputs, sent_id, tgt_eos, subword_option):
  """Given batch decoding outputs, select a sentence and turn to text."""
  if tgt_eos: tgt_eos = tgt_eos.encode("utf-8")
  output = nmt_outputs[sent_id, :].tolist()
  if tgt_eos and tgt_eos in output:
