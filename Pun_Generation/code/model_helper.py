...
VOCAB_SIZE_THRESHOLD_CPU = 105000
...
def create_train_model(
  ...
  if hparams.reward_prefix:
    reward_file = "%s" % (hparams.reward_prefix)
  ...
  with graph.as_default(), tf.container(scope or "train"):
    ...
    if hparams.reward_prefix:
        def parse_fnc(line):
            return tf.string_to_number(line, tf.float32)
        reward_dataset = tf.data.TextLineDataset(reward_file)
        reward_dataset = reward_dataset.map(map_func=parse_fnc)
...
def _create_or_load_embed(embed_name, vocab_file, embed_file,
  ...
  if vocab_file and embed_file:
    ...
    #with tf.Session() as sess:
     # sess.run(embedding)
      #save_path = saver.save(sess, "/home/yuzw/pun/nmt/final_data/embedding.npy")
      #print ("save_path,",save_path)
    #with open('/home/yuzw/pun/nmt/final_data/embedding','w+') as f:
     #   f.write()
  return embedding
