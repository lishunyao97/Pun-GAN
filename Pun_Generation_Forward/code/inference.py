...
def multi_worker_inference(infer_model,
  ...
  with tf.Session(
    ...
    #print (sess.run(tf.GraphKeys.GLOBAL_VARIABLES))
     #saver = tf.train.Saver()
     #sess=tf.Session()
     #sess.run(infer_model.model.embedding_encoder)
     #saver.save(sess, "/home/yuzw/pun/nmt/inference/emb.npz")
     #print("Model saved in path:")
    # Decode
    utils.print_out("# Start decoding multi_worker_inference")
    nmt_utils.decode_and_evaluate(
        "infer",
        loaded_infer_model,
        sess,
